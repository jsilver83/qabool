...
== Students
=== Student Registration and Preview
...
. Add new caption/label called gender: only for male.
. Add new two fields called : Qudurat and Tahseel grades.
. Add note below mobile number (to receive a message you have to activate promotion message from your network provider).
. In the student dashboard move up student name and change caption name: "اسم الطالب الثنائي".
. The students should preview their entries before submitting the form.
...
. Partial admitted students can enter vehicle information
(driving license and vehicle registration) and also upload the documents.
...
. Admitted student can not change the reservation once he select and submit the form.
...
Dependencies::	login screen and Q.00000 committee.
...
. Admitted student can answer the questionnaire that related to find a roommate.
...
Dependencies::	login screen , Q.03 and Q.09 committee.
...
Rationale:: These KFUPM IDs will distribute to Admitted students and it will be
...
It contains only two fields (student ID and student level).
...
. Committee should be able to correct student full names in both languages (only full names).
. All fields should be "read only" except full names.
...
Dependencies::	Q.01 , admission screen and login screen.
...
Fit Criteria::
. Admission office should be able to apply cut-off points based on students GPA
and their types (S, M and N ) and student nationalities.
...
. Display list of students who want to get car stickers.
