...
def important_dates(context):
    ...
    if cache.get('important_dates') is None:
        important_dates_list = ImportantDateSidebar.objects.filter(show=True)
        cache.set('important_dates', important_dates_list, 15 * 60)
    else:
        important_dates_list = cache.get('important_dates')

    return {
        'important_dates': important_dates_list,
    }
